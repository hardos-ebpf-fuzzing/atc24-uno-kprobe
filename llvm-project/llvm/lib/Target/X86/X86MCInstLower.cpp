...
void X86AsmPrinter::LowerKCFI_CHECK(const MachineInstr &MI) {
  ...
  assert((std::next(MI.getIterator())->isCall() ||
          std::next(MI.getIterator())->getOpcode() == X86::KPROBE_OPT) &&
...
void X86AsmPrinter::LowerKPROBE_OPT(const MachineInstr &MI) {
  NoAutoPaddingScope NoPadScope(*OutStreamer);

  // Need a 5-byte nop region for runtime-patching
